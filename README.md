# Fine-tuning Whisper models using your personal records for OpenHAB.

These is a fork of [whisper-finetune](https://github.com/vasistalodagala/whisper-finetune) focused on using docker for creating personal fine-tuned ggml models from the local records and property files generated by the OpenHAB WhisperSTT add-on.

Note that Dockerfile.cuda was built from the cuda official containers therefore under the [Nvidia Container License](https://developer.download.nvidia.com/licenses/NVIDIA_Deep_Learning_Container_License.pdf).
...
```sh
docker build . -t yourname/whisper-finetune-oh:cpu -f Dockerfile.cpu
```

### GPU image

```sh
docker build . -t yourname/whisper-finetune-oh:cu113 -f Dockerfile.cuda
```

# Original whisper-finetune README follows
